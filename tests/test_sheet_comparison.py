...
from comparator.sameSheet import compare_sheet_properties
...
class TestComparison:
    ...
    def test_same_sheet(self):
        ...
        assert compare_sheet_properties(sheet1, sheet2) is True

    def test_different_values(self):
        sheet1: Sheet = Sheet(sheet_name="Sheet1", rows=1, columns=6, values=["A", "B", "C"])
        sheet2: Sheet = Sheet(sheet_name="Sheet1", rows=1, columns=6, values=["m", "k", "D"])

        assert compare_sheet_properties(sheet1, sheet2) is False
